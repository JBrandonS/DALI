...
class parth_exact(Likelihood):
    ...
    def initialize(self):
        ...
        self.obs_fid_errors['tau'] = (self.cosmoFid['tau'], self.tau_sigma)
    ...
    def logp(self, _derived=None, **kwargs):        
        ...
        for k,v in self.obs_fid_errors.items():
            self.logger.log(DEBUG, f'key: {k}, (fid, error): {v}, point: {point[k].values[0]}, dchi2:{(point[k].values[0] - float(v[0]))**2}')
            chi2 += (point[k].values[0] - float(v[0]))**2
        return -chi2 / (2 * self.sigma2)
